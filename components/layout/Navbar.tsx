...
export default function Navbar({ userEmail, userId }: NavbarProps) {
  ...
  const handleManageCohort = (cohortId: number) => {
    playClickSound();
    const setCookie = (name: string, value: string, days = 365) => {
      const date = new Date();
      date.setTime(date.getTime() + (days * 24 * 60 * 60 * 1000));
    ...
    setCookie('active_cohort_id', String(cohortId));
    window.location.href = '/cohort-admin';
  };
                      ...
                      <>
                        ...
                        {cohorts.map((c) => (
                          <div key={c.id} className="flex items-center gap-0.5 pl-1 pr-1">
                            <SelectItem value={String(c.id)} className="text-xs pl-3 flex-1 min-w-0">
                            ...
                            {c.role === 'admin' && (
                              <button
                                ...
                                onClick={(e) => { e.stopPropagation(); e.preventDefault(); handleManageCohort(c.id); }}
                                title="Kelola Komunitas"
                                className="flex-shrink-0 p-1.5 rounded-md text-slate-400 hover:text-primary hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
                              >
                        ...
                        <div key={c.id} className="flex items-center gap-0.5 pl-1 pr-1">
                          ...
                          {c.role === 'admin' && (
                            <button
                              type="button"
                              onPointerDown={(e) => e.stopPropagation()}
                              onClick={(e) => { e.stopPropagation(); e.preventDefault(); setIsMobileOpen(false); handleManageCohort(c.id); }}
                              title="Kelola Komunitas"
                              className="flex-shrink-0 p-1.5 rounded-md text-slate-400 hover:text-primary hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
                            >
                              <Settings className="h-3.5 w-3.5" />
